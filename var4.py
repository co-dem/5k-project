    ...
    for i, j in modes.items():
        ...
        for i in my_text:
            ...
            try:
                ...
                print(f'{Fore.RED}ERROR: Content parameter is invalid')
            headers = {'api-key': '1684460571197x967751436416797200','Content-Type': 'application/json'}
            while con:
...
def send_to_jasper_for_lists(txt = '', mode = 'fuh'):
    ...
    if mode == 'fuh':
        ...
        for comm in ctts:
            ...
            comm = '{0}\n'.format(comm.replace('\n', ' '))
            driver.find_element(By.ID, "chat-user-input").send_keys(f"{comm}\n")
            time.sleep(17)
            txt_elems = driver.find_element(By.XPATH, ".transition-opacity:nth-child(4)")
            fr.append(txt_elems.text)

    if mode == 'fuh':
        try                          : headers = driver.find_element(By.CSS_SELECTOR, '.transition-opacity:nth-child(4)')
