...
def getCommands(path=''):
    ...
    #* cheking if available data entered to argument "path"
    if not bool(path):
...
def chooseHeadersToDelete(uh: list, h: list):
    ...
    for indx, elem in enumerate(commands_from_txt_file):
        for j in dheaders:
            if j in elem:
                print(j, i)
                del commands_from_txt_file[indx]
    print(len(commands_from_txt_file))

    quit()

def changingCommandVariable(cvar: str): #cvar - Command Variable
    pass
...
def main():
    ...
    try:
        ...
        print(f'{Fore.RED}[log](main - error): {e}{Fore.WHITE}')
